...
export default function DashboardGrid() {
  ...
  const totalValue = useMemo(() => {
    return Object.entries(holdings).reduce((total, [id, qty]) => {
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-gray-400 text-[10px] font-black uppercase tracking-[0.3em]">
                {saving ? 'Syncing...' : 'Total Portfolio Value'}
              </h2>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {loading && watchlist.length > 0 ? (
          <div className="flex flex-col items-center justify-center mt-32">
            ...
            <p className="text-gray-500 font-mono text-[10px] uppercase tracking-[0.4em] animate-pulse">Loading...</p>
          </div>
        ) : (
            ...
            {watchlist.map((id) => (
              <div key={id} className="transform hover:scale-[1.02] transition-all duration-300">
                <ChartCard 
                  ...
                  qty={holdings[id] || 0}
                  onQtyChange={(newQty) => handleQtyChange(id, newQty)}
                />
              </div>
            ))}
          </div>
